...
\[
    ...
    \prod_{l=1}^{d}
    \left(
        \prod_{(a_j,v_j)\in Q_l}H(Fid_l\Vert a_j)^{v_j}
    \right)^{\beta_l}.
\]
...
the \(N=1\) specialization of the original multi-copy and multi-provider
formulation:
\[
...
diagnosis from the original MPA setting is omitted because the comparison
model sets \(N=1\).
...
\(a\) is chosen significantly larger than both \(v\) and \(K\)
...
\]
and per-file coefficients \(v_{i,j}=\pi_{\pi_{k_2}(i)}(j)\).  Using the
trapdoor, they decrypt matching index rows and derive the union of files that
contain at least one queried keyword.  This is the source construction's
OR-style matching semantics: \(R=R_1\cup\cdots\cup R_\gamma\) on the cloud
side and \(T=T_1\cup\cdots\cup T_\zeta\) on the auditor side.  Let \(R\)
denote the union set found by the cloud and \(T\) the union set found by the
auditor.

The cloud computes the aggregate data value and aggregate authenticator
\[
    \mu=\sum_{i\in R}\sum_{j\in Q} v_{i,j}m_{i,j},
    \qquad
    T_{auth}=\prod_{i\in R}\prod_{j\in Q}\sigma_{i,j}^{v_{i,j}}.
\]
It samples \(r\in\Zq^\ast\), sets \(R_u=u^r\), and masks the aggregate data as
\[
    \mu_1=\mu-rh_1(T_w,R_u).
\]
The proof is
\[
    \mathsf{Proof}=(T_{auth},\mu_1,R_u).
\]
\paragraph{Implementation note.}
The original MiaoSCIS26 text writes \(R=g^r\).  For a reproducible
implementation we use \(R_u=u^r\), so that the masking term is algebraically
consistent with the data factor \(u^{m_{i,j}}\) inside each authenticator:
\[
    u^{\mu_1}R_u^{h_1(T_w,R_u)}
    =u^{\mu-rh_1(T_w,R_u)}(u^r)^{h_1(T_w,R_u)}
    =u^\mu .
\]
Thus \(R_u=u^r\) is an implementation-oriented consistency correction rather
than the paper's literal notation.
...
e\!\left(
    ...
    \cdot u^{\mu_1}\cdot R_u^{h_1(T_w,R_u)},
    y
\right).
\]
...
To add a file \(F_z\), the data owner inserts the new file column, namely the
\((v+1)\)-th column, in \(I_{a\times a}\), recomputes the encrypted affected
rows, calculates
\[
